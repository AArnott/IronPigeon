...
public class ChannelTests : TestBase, IAsyncLifetime, IDisposable
{
    ...
    public async Task PostAndReceiveAsync()
    {
        ...
        List<IronPigeon.Relay.InboxItem> receivedMessages = await this.channel2.ReceiveInboxItemsAsync(cancellationToken: this.TimeoutToken).ToListAsync(this.TimeoutToken);
    ...
    public async Task PayloadTamperingTests()
    {
        for (int i = 0; i < 16; i++)
        {
            ...
            await foreach (IronPigeon.Relay.InboxItem receivedMessages in this.channel2.ReceiveInboxItemsAsync(cancellationToken: this.TimeoutToken))
            {
