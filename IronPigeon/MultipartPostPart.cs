namespace IronPigeon {
	...
	public class MultipartPostPart : IDisposable {
		...
		public long Length {
			get {
				if (this.Content == null || this.Content.Length < 0) {
					throw new InvalidOperationException("Streams must have a known length.");
				}
		...
		/// <returns>The constructed part.</returns>
		public static MultipartPostPart CreateFormFilePart(string name, string filePath, Stream fileStream, string contentType) {
			Requires.NotNullOrEmpty(name, "name");
		...
		internal async Task SerializeAsync(StreamWriter streamWriter) {
			...
			await streamWriter.WriteAsync(String.Format(CultureInfo.InvariantCulture, "{0}: {1}", ContentDispositionHeader, this.ContentDisposition));
			foreach (var pair in this.ContentAttributes) {
				await streamWriter.WriteAsync(String.Format(CultureInfo.InvariantCulture, "; {0}=\"{1}\"", pair.Key, pair.Value));
			}

			await streamWriter.WriteLineAsync();
			foreach (string headerName in this.PartHeaders) {
				await streamWriter.WriteLineAsync(String.Format(CultureInfo.InvariantCulture, "{0}: {1}", headerName, this.PartHeaders[headerName]));
			}

			await streamWriter.WriteLineAsync();
			await streamWriter.FlushAsync();
			await this.Content.CopyToAsync(streamWriter.BaseStream);
		}
